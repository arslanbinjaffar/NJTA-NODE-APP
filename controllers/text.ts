...
/*
  @desc POST create text in page
  @route POST api/v1/create/text/:userId/:pageId
  @access Public
 */
const createText = async (req: Request, res: Response, next: NextFunction) => {
  ...
  try {
    ...
    if (blockType !== "text") throw new Error("Block type not recognized");
...
/*
  @desc PATCH update single text
  @route PATCH api/v1/text/:userId/:pageId/:blockId
  @access Public
 */
...
/*
  @desc DELETE delete single text
  @route DELETE api/v1/text/:userId/:pageId/:blockId
  @access Public
 */
const deleteText = async (req: Request, res: Response, next: NextFunction) => {
  const { userId, pageId, blockId } = req.params;
...
export { createText, updateText, deleteText };
